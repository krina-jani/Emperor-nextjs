'use client';

import React, { useState } from 'react';
import Link from 'next/link';
import { footerLinks } from '../../data/navigation';
import { cn } from '../../lib/utils';
import styles from './Footer.module.css';
import { Mail, Send } from 'lucide-react';

const LinkedinIcon = ({ size = 18 }: { size?: number }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" /><rect width="4" height="12" x="2" y="9" /><circle cx="4" cy="4" r="2" /></svg>
);

const TwitterIcon = ({ size = 18 }: { size?: number }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z" /></svg>
);

const GithubIcon = ({ size = 18 }: { size?: number }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" /><path d="M9 18c-4.51 2-5-2-7-2" /></svg>
);

export const Footer: React.FC = () => {
  const [email, setEmail] = useState('');
  const [isSubmitted, setIsSubmitted] = useState(false);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!email) return;
    setIsSubmitted(true);
    setEmail('');
    setTimeout(() => setIsSubmitted(false), 4000);
  };

  return (
    <footer className={styles.footer}>
      <div className={cn('container', styles.container)}>
        {/* Company Identity */}
        <div className={styles.identityCol}>
          <Link href="/" className={styles.logo}>
            <span className={styles.logoIcon}>👑</span>
            <span className={styles.logoText}>
              EMPEROR<span className={styles.accentText}>.</span>
            </span>
          </Link>
          <p className={styles.tagline}>
            Architecting high-performance digital systems, custom machine learning integrations, and robust cloud configurations for global enterprise scale.
          </p>
          <div className={styles.socials}>
            <a href="https://linkedin.com" target="_blank" rel="noreferrer" className={styles.socialLink} aria-label="LinkedIn">
              <LinkedinIcon size={18} />
            </a>
            <a href="https://twitter.com" target="_blank" rel="noreferrer" className={styles.socialLink} aria-label="Twitter">
              <TwitterIcon size={18} />
            </a>
            <a href="https://github.com" target="_blank" rel="noreferrer" className={styles.socialLink} aria-label="GitHub">
              <GithubIcon size={18} />
            </a>
          </div>
        </div>

        {/* Sitemap Navigation Links */}
        <div className={styles.linksCol}>
          <h3 className={styles.heading}>Services</h3>
          <ul className={styles.list}>
            {footerLinks.services.map((item) => (
              <li key={item.href} className={styles.item}>
                <Link href={item.href} className={styles.link}>
                  {item.label}
                </Link>
              </li>
            ))}
          </ul>
        </div>

        <div className={styles.linksCol}>
          <h3 className={styles.heading}>Solutions</h3>
          <ul className={styles.list}>
            {footerLinks.solutions.map((item) => (
              <li key={item.href} className={styles.item}>
                <Link href={item.href} className={styles.link}>
                  {item.label}
                </Link>
              </li>
            ))}
          </ul>
        </div>

        <div className={styles.linksCol}>
          <h3 className={styles.heading}>Company</h3>
          <ul className={styles.list}>
            {footerLinks.company.map((item) => (
              <li key={item.href} className={styles.item}>
                <Link href={item.href} className={styles.link}>
                  {item.label}
                </Link>
              </li>
            ))}
          </ul>
        </div>

        {/* Newsletter Section */}
        <div className={styles.newsletterCol}>
          <h3 className={styles.heading}>Stay Informed</h3>
          <p className={styles.newsletterDesc}>
            Subscribe to our newsletter for insights into AI modeling, serverless scaling, and custom web architectures.
          </p>
          <form className={styles.form} onSubmit={handleSubmit}>
            <div className={styles.inputWrapper}>
              <Mail className={styles.mailIcon} size={16} />
              <input
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="Enter email address"
                required
                className={styles.input}
              />
            </div>
            <button type="submit" className={styles.submitBtn} aria-label="Subscribe">
              <Send size={16} />
            </button>
          </form>
          {isSubmitted && (
            <span className={styles.successMsg}>
              Successfully subscribed to our list!
            </span>
          )}
        </div>
      </div>

      {/* Bottom Footer Section */}
      <div className={styles.bottomBar}>
        <div className={cn('container', styles.bottomContainer)}>
          <span className={styles.copyright}>
            &copy; {new Date().getFullYear()} Emperor Smart Solution. All rights reserved.
          </span>
          <div className={styles.legalLinks}>
            {footerLinks.legal.map((item) => (
              <Link key={item.href} href={item.href} className={styles.legalLink}>
                {item.label}
              </Link>
            ))}
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
